				...
				<table className="min-w-full divide-y divide-gray-200">
					<thead className="bg-gray-50">
						<tr>
							...
							<th className="px-responsive py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
								Action
							</th>
						</tr>
					</thead>
					<tbody className="bg-white divide-y divide-gray-200">
						{loaderData.users.map((user) => (
							<tr key={user.id} className="hover:bg-gray-50">
								...
								<td className="px-responsive py-4 whitespace-nowrap">
									<span
										className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.status === "suspended" ? "bg-red-100 text-red-800" : "bg-blue-100 text-blue-800"}`}
									>
										{user.status || "active"}
									</span>
								</td>
								...
								<td className="px-responsive py-4 whitespace-nowrap text-right text-sm font-medium">
									<Link
										to={`/users/${user.id}`}
