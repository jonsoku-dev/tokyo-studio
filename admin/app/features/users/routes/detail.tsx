...
export async function action({ request, params }: Route.ActionArgs) {
	...
	if (intent === "reset_roadmap") {
		...
		const { requireAdmin } = await import(
			"~/features/auth/utils/session.server"
		);
		const { resetUserRoadmap } = await import(
			"~/features/roadmap/services/admin-roadmap.server"
		);
					...
					<div className="flex gap-2">
						...
						<Form
							...
							onSubmit={(e) => {
								if (
									!confirm(
										"This will delete all current roadmap tasks and progress for this user. Are you sure?",
									)
								) {
									e.preventDefault();
								}
			...
			<div className="grid grid-cols-1 gap-responsive lg:grid-cols-3">
				...
				<div className="bg-white p-responsive rounded-xl shadow-sm border border-gray-200">
					...
					<dl className="space-y-2 text-sm">
						...
						<div className="flex justify-between">
							<dt className="text-gray-500">Job Family</dt>
							<dd>{profile?.jobFamily || "-"}</dd>
						</div>
						<div className="flex justify-between">
						...
						<div className="flex justify-between">
							...
							<dd>{profile?.targetCity || "-"}</dd>
						</div>
					</dl>
				</div>

				{/* Payments */}
				<div className="bg-white p-responsive rounded-xl shadow-sm border border-gray-200 lg:col-span-2">
				...
				{/* Mentoring */}
				<div className="bg-white p-responsive rounded-xl shadow-sm border border-gray-200 lg:col-span-3">
