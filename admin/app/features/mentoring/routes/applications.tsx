...
import { Check, X, FileText, ChevronRight, Download } from "lucide-react";
...
export async function loader({ request }: Route.LoaderArgs) {
	...
	if (selectedId) {
		...
		if (app && app.verificationFileUrl) {
...
export async function action({ request }: Route.ActionArgs) {
	...
	if (intent === "approve") {
		await applicationService.processApplication(userId, applicationId, "approved");
	} else if (intent === "reject") {
		await applicationService.processApplication(userId, applicationId, "rejected", rejectionReason);
	}
...
export default function AdminApplicationsPage({ loaderData }: Route.ComponentProps) {
	...
	return (
		<div className="p-8">
			<h2 className="text-2xl font-bold mb-6 text-gray-900">Mentor Applications</h2>
						...
						>
							<div className="flex justify-between items-start mb-2">
								<div>
									...
									<p className="text-sm text-gray-500">{app.jobTitle} @ {app.company}</p>
								</div>
					...
					{selectedApp ? (
						<div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
							<div className="flex justify-between items-start mb-6">
								<div>
									<h2 className="text-xl font-bold text-gray-900">{selectedApp.user.name}</h2>
									<p className="text-gray-500">{selectedApp.jobTitle} @ {selectedApp.company}</p>
								</div>
							...
							<div className="space-y-6">
								<Section title="Professional Background">
									<p className="text-gray-700"><span className="text-gray-500">Experience:</span> {selectedApp.yearsOfExperience} Years</p>
									<p className="text-gray-700"><span className="text-gray-500">LinkedIn:</span> <a href={selectedApp.linkedinUrl || "#"} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">{selectedApp.linkedinUrl || "N/A"}</a></p>
								</Section>

								<Section title="Expertise & Languages">
									<div className="flex flex-wrap gap-2 mb-2">
										{selectedApp.expertise.map(t => <span key={t} className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">{t}</span>)}
									</div>
									<div className="flex flex-wrap gap-2">
										{selectedApp.languages.map(t => <span key={t} className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs">{t}</span>)}
									</div>
								</Section>
								...
								<Section title="Verification Document">
									<div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
										...
										<div className="flex-1 overflow-hidden">
											<p className="text-sm font-medium truncate text-gray-900">{selectedApp.verificationFileUrl}</p>
									...
									<p className="text-xs text-yellow-600 mt-2">* This link is generated for admin access only.</p>
								</Section>
									...
									<div className="pt-6 border-t border-gray-200 flex gap-4 justify-end">
										<Form method="post" className="flex gap-2 items-center">
											...
											<div className="flex items-center gap-2">
                                                <input type="text" name="rejectionReason" placeholder="Rejection reason (if rejecting)" className="border rounded px-2 py-1 text-sm w-48" />
                                                <button 
                                                    type="submit" 
                                                    name="intent" 
                                                    value="reject" 
