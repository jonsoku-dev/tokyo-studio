...
const isS3Configured = AWS_ACCESS_KEY_ID && AWS_SECRET_ACCESS_KEY && AWS_BUCKET_NAME;
...
export const storageService = {
    async getPresignedUrl(key: string, contentType: string) {
        if (!isS3Configured || !s3Client) {
            // Mock Mode or Error
            console.warn("StorageService: AWS S3 Credentials missing. Using Mock Mode (returning fake URL).");
