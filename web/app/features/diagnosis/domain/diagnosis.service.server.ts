	...
	calculateRecommendations(profile: InsertProfile) {
		...
		if (["N1", "N2", "Native"].includes(jpLevel)) {
			...
			recommendations.push({
				...
				type: "channel",
				description:
					"Your Japanese level allows you to compete with local candidates. Apply directly to companies using Wantedly or Green.",
			...
			recommendations.push({
				...
				type: "channel",
				description:
					"Focus on Rakuten, LINE, Mercari, and startups that operate in English.",
			...
			recommendations.push({
				...
				type: "channel",
				description:
					"Use your Korean background as leverage. Connect with agents specializing in KR-JP bridging.",
		...
		if (years >= 5 || level === "senior" || level === "lead") {
			recommendations.push({
				...
				type: "visa",
				description:
					"You likely qualify for the HSP (Highly Skilled Professional) Visa (80 points+).",
		...
		if (jobFamily === "frontend") {
			recommendations.push({
				...
				type: "action",
				description:
					"Japanese companies value modern stack (Next.js/React) and design sensibility. Polish your portfolio site.",
			});
			...
			recommendations.push({
				...
				type: "action",
				description:
					"Highlight your AWS/GCP experience and system design capabilities. Coding tests are common.",
			});
		}
