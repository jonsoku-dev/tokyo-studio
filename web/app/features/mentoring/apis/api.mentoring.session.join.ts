 ...
 */
export const loader = loaderHandler(async ({ params, request }: LoaderFunctionArgs) => {
	// 1. Authenticate user
	...
	if (sessionRecord.status !== "confirmed") {
		throw new BadRequestError(`Session is not confirmed (status: ${sessionRecord.status})`);
	}
	...
	if (!sessionRecord.meetingUrl) {
		...
		throw new ServiceUnavailableError("Meeting URL is not yet available. Please contact support.");
	}
