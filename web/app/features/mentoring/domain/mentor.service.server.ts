...
import { mentors, mentorAvailability } from "~/shared/db/schema";
...
export const mentorService = {
    ...
    async getAvailability(mentorId: string) {
        return await db.select().from(mentorAvailability).where(eq(mentorAvailability.mentorId, mentorId));
    },

    async updateAvailability(mentorId: string, dayOfWeek: string, startTime: string, endTime: string) {
        // Simple add for now. Ideally should handle overlap or replace.
        // For MVP, we'll just insert. Logic to clear can be added later.
        return await db.insert(mentorAvailability).values({
            mentorId,
            dayOfWeek,
            startTime,
            endTime
    ...
    async clearAvailability(mentorId: string, dayOfWeek: string) {
